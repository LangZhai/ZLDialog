$(function () {
    ...
    $.support.cors = true;
    $.ajax({
        url: (location.protocol.match(/^http/) ? location.protocol : 'https:') + '//langzhai.github.io/ZLDialog/attr.json',
        dataType: 'json'
    }).done(function (data) {
        $('#attrTable').html($('#template').template(data, {nested: '#template-attr,#template-sub'}));
    });
    $.ajax({
        url: (location.protocol.match(/^http/) ? location.protocol : 'https:') + '//langzhai.github.io/ZLDialog/README.md'
    }).done(function (data) {
    ...
    $('#licenseA').on('click', function () {
        $.ajax({
            url: (location.protocol.match(/^http/) ? location.protocol : 'https:') + '//langzhai.github.io/ZLDialog/LICENSE'
        }).done(function (data) {
            $.dialog({
